...
void test_dry_run_stow(void) {
    ...
    ASSERT(!file_exists(target_cfg),
           "Dry run stow must not modify disk or create symlinks");
...
void test_symlink_health_check(void) {
    ...
    snprintf(broken_link, sizeof(broken_link), "%s/broken.symlink",
             tmp_dotfiles);
...
void test_unfold_directory_symlinks(void) {
    ...
    ASSERT(symlink(pkg_sub, target_sub) == 0,
           "Should create directory symlink in target");
    ...
    ASSERT(is_symlink(target_child),
           "target/sub/file1.txt should be a symlink");
...
void test_package_stow_status(void) {
    ...
    ASSERT(get_package_stow_status(tmp_target, tmp_dotfiles, "statpkg") ==
               STOW_STATUS_UNSTOWED,
...
void test_native_unstow_edited_user_file(void) {
    ...
    ASSERT(is_symlink(target_file),
           "target_file should be a symlink initially");
    ...
    ASSERT(unstow_package(tmp_dotfiles, tmp_target, "usrpkg", false) == 0,
           "Unstow should succeed");

    ASSERT(file_exists(target_file),
           "User file should be preserved during unstow");
...
void test_native_unstow_recursive_directory_cleanup(void) {
    ...
    ASSERT(mkdir_p(deep_dir, 0755) == 0,
           "Should create deep package directory");
    ...
    snprintf(target_file, sizeof(target_file), "%s/dir1/dir2/dir3/deep.conf",
             tmp_target);

    ASSERT(is_symlink(target_file), "deep.conf should be stowed as symlink");

    char unrelated_file[PATH_MAX * 2];
    snprintf(unrelated_file, sizeof(unrelated_file), "%s/unrelated.txt",
             target_dir1);
    ...
    ASSERT(unstow_package(tmp_dotfiles, tmp_target, "recpkg", false) == 0,
           "Unstow should succeed");

    ASSERT(!file_exists(target_file), "deep.conf should be removed");
    ASSERT(!is_dir(target_dir3),
           "dir3 should be cleaned up as empty directory");
    ASSERT(!is_dir(target_dir2),
           "dir2 should be cleaned up as empty directory");
    ASSERT(is_dir(target_dir1),
           "dir1 should NOT be cleaned up because it contains unrelated.txt");
...
void test_native_stow_ignore_patterns(void) {
    ...
    snprintf(target_normal, sizeof(target_normal), "%s/normal.conf",
             tmp_target);
    snprintf(target_log, sizeof(target_log), "%s/app.log", tmp_target);
    snprintf(target_secret, sizeof(target_secret), "%s/ignored_dir/secret.key",
             tmp_target);

    ASSERT(is_symlink(target_normal), "normal.conf should be stowed");
    ASSERT(!file_exists(target_log),
           "app.log should be ignored and not created");
    ASSERT(!file_exists(target_secret),
           "ignored_dir/secret.key should be ignored and not created");
...
void test_dynamic_package_conflicts(void) {
    ...
    ASSERT(is_package_stowed(tmp_target, tmp_dotfiles, "nvim"),
           "nvim package should be stowed");

    // Stow pkg2 (nvim-headless) -> triggers dynamic collision detection and
    // auto-unstows nvim
    ASSERT(stow_package(tmp_dotfiles, tmp_target, "nvim-headless", false,
                        false) == 0,
