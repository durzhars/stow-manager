...
void test_handle_mutual_exclusions(void) {
    ...
    char pkg_a_dir[PATH_MAX], pkg_b_dir[PATH_MAX];
    ...
    if (fa) { fprintf(fa, "content A\n"); fclose(fa); }
    FILE *fb = fopen(file_b, "w");
    if (fb) { fprintf(fb, "content B\n"); fclose(fb); }
    ...
    ASSERT(!is_package_stowed(tmp_target, tmp_dotfiles, "pkgB"), "pkgB should be unstowed after mutual exclusion handling");
...
void test_unfold_directory_symlinks(void) {
    ...
    char pkg_dir[PATH_MAX], pkg_sub[PATH_MAX], file1[PATH_MAX];
    snprintf(pkg_dir, sizeof(pkg_dir), "%s/mypkg", tmp_dotfiles);
...
void test_package_stow_status(void) {
    ...
    char f1[PATH_MAX], f2[PATH_MAX], f_deps[PATH_MAX];
    ...
    FILE *fp1 = fopen(f1, "w"); if (fp1) { fprintf(fp1, "1\n"); fclose(fp1); }
    FILE *fp2 = fopen(f2, "w"); if (fp2) { fprintf(fp2, "2\n"); fclose(fp2); }
    FILE *fpd = fopen(f_deps, "w"); if (fpd) { fprintf(fpd, "REQUIRED=\"\"\n"); fclose(fpd); }

    // Initially 0 files stowed
    ASSERT(get_package_stow_status(tmp_target, tmp_dotfiles, "statpkg") == STOW_STATUS_UNSTOWED, "Should be UNSTOWED initially");

    // Stow file1 only -> PARTIAL
    char tf1[PATH_MAX];
    snprintf(tf1, sizeof(tf1), "%s/.file1", tmp_target);
    symlink(f1, tf1);
    ASSERT(get_package_stow_status(tmp_target, tmp_dotfiles, "statpkg") == STOW_STATUS_PARTIAL, "Should be PARTIAL when 1/2 non-ignored files stowed");

    // Stow file2 as well -> STOWED (even though .stowdeps is not stowed)
    char tf2[PATH_MAX];
    snprintf(tf2, sizeof(tf2), "%s/.file2", tmp_target);
    symlink(f2, tf2);
    ASSERT(get_package_stow_status(tmp_target, tmp_dotfiles, "statpkg") == STOW_STATUS_STOWED, "Should be STOWED when 2/2 non-ignored files stowed");
